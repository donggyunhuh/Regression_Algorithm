...
이 레포트에서는 $a$값, 즉 에러값이 최소일때 최적해(_회귀식의 기울기_)를 찾는 것을 목표로 한다.
...
유전 알고리즘을 적용할 데이터의 적합도를 구하고 적합도를 용해 데이터를 정렬한다.

이 데이터를 교차시키며 교차시 돌연변이 데이터가 발생할 확률을 부여한다.

탄생한 자식 데이터중 적합도 평가를 통해 우성인 데이터를 찾고 부모와 교환한다.

이과정을 계속 반복하며 최적화 값을 찾는다.

인접한 두점의 평균값을 구하고 구한 평균값을
