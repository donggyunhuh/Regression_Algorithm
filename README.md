...
_노상규, 1998, 「유전자 알고리즘을 이용한 변수 선택법」_ 을 참고하였다.
...
이 레포트에서는 회귀식을 찾아내기 위해 최적화 알고리즘 '**유전 알고리즘**' 을 사용한다.

# 유전 알고리즘

- 유전 알고리즘을 이용한 회귀식 탐색 과정

유전 알고리즘은

**선택(Selection)**, **교차(Crossover)**, **변이(Mutate)**, **대치(Replace)**, **반복(Loop)**의 과정을 가진다.

간단히 정리하면, 적합도를 구한 다음 유전자를 적합도에 맞게 표현하고 정리한다.

돌연변이 유전자가 탄생할 수 있는 가능성을 부여하고 반반씩 교차시켜 탄생한 자식 유전자 중 적합도가 높은 자식 유전자를 선택한다.
...
- 다시 회귀식을 설명하면
  ...
  회귀선을 구하기 위해 회귀선을 $y=ax+b$, $f(x)=y$ 라고 가정한다.  
   구한 데이터 값 중에서 임의로 한 데이터 값을 좌표의 형태로 나타낸다.  
   예를 들어, 아래 그래프에서 $(1657,229.2)$ 점과 회귀선 $f(x)=ax+b$ 사이의 거리는 $f(1650) - 229.2$ 이다. 이를 통계학 용어로 잔차라고 부른다.  
...
이과정을 계속 반복하며 최적화 값(최적해, 여기선 기울기 $a$)을 찾는다.
